...
class SFSORT:
    ...
    def update(self, detections):
        ...
        if high_score.any():
            ...
            if track_pool:
                ...
                matches, unmatched_tracks, unmatched_detections = (
                    self.linear_assignment(cost, mth)
                )
        ...
        if intermediate_score.any():
            if len(unmatched_tracks):
                ...
                cost = self.calculate_cost(
                    unmatched_track_pool, possible_boxes, iou_only=True
                )
                matches, unmatched_tracks, unmatched_detections = (
                    self.linear_assignment(cost, self.match_th_second)
                )
                # Update/Activate matched tracks
                for track_idx, detection_idx in matches:
        ...
        for track in next_lost_tracks:
            if track not in self.lost_tracks:
                ...
                if (self.l_margin < u < self.r_margin) and (
                    self.t_margin < v < self.b_margin
                ):
    ...
    def calculate_cost(tracks, boxes, iou_only=False):
        ...
        inner_diag = np.abs(centerx1[:, None] - centerx2) + np.abs(
            centery1[:, None] - centery2
        )
